...
//        qDebug() << "Forwarding signal" << slot.signalIdx << "to puppet" << (void*)slot.puppetPtr << "with args" << parameters;
        ...
        q.propertyValue = slot.object->metaObject()->property(slot.propertyIndex).read(slot.object);
...
SignalProxy::SignalProxy(QObject *parent) :
    QObject(parent)
{
    RPCPeer *p = qobject_cast<RPCPeer*>(parent);
    if (!p)
...
RPCPeer *SignalProxy::rpcPeer() const
{
    return reinterpret_cast<RPCPeer*>(parent());
}
