...
//        qDebug() << "Reading remote property" << this->metaObject()->property(id).name();
        if (now() - prop.lastQuery > 10) {
            MSG_BODY(ReadProperty) q;
            q.id = id;
            q.objPtr = m_ptr;
            ...
            prop.lastQuery = now();
            prop.value = QVariant(this->metaObject()->property(id).type(), arguments[0]);
//            qDebug() << "returning remotely read property";
        } else {
//            qDebug() << "returning cached property";
    ...
    case QMetaObject::WriteProperty: {
        ...
        q.propertyIndex = id;
        q.objPtr = m_ptr;
        q.value = QVariant(m_propertyCache[id].type, arguments[0]);
        qDebug() << "Writing property" << metaObject()->property(id).name() << "at" << (void*)m_ptr << "value" << q.value;
