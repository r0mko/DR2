...
void RPCEngine::h_WriteProperty(Peer *peer, Message *message)
{
    ...
    QObject *obj = reinterpret_cast<QObject *>(msg->objPtr);
    QMetaProperty p = obj->metaObject()->property(msg->propertyIndex);
    p.write(obj, msg->value);
    //    qDebug() << "Updated property" << msg->propertyIndex << obj->metaObject()->property(msg->propertyIndex).name() << "to" << msg->value;
    if (p.notifySignalIndex() > 0) {
        //        qDebug() << "emitting notify signal" << p.notifySignal().methodSignature();
        p.notifySignal().invoke(obj, Qt::QueuedConnection);
    }
}
...
void RPCEngine::h_SignalDisconnected(Peer *peer, Message *message)
{
    ...
    RPCEngine *e = RPCEngine::engine();
    RPCPeer *p = e->m_peers.value(peer);
    Q_ASSERT(p != nullptr); // FIXME: crash here!
    QObject *sender = reinterpret_cast<QObject*>(msg->remotePtr);
...
void RPCEngine::h_EmitSignal(Peer *peer, Message *message)
{
    ...
    RPCEngine *e = RPCEngine::engine();
    QMetaObject::invokeMethod(e, "emitPuppetSignalHelper", Qt::QueuedConnection,
                              Q_ARG(quint64, msg->puppetPtr),
                              Q_ARG(int, msg->signalIdx),
                              Q_ARG(QVariantList, msg->arguments));
    //    qDebug() << "Emitted signal in different thread" << QThread::currentThread();
    //    e->emitPuppetSignalHelper(p, msg->signalIdx, msg->arguments);
...
void RPCEngine::h_EmitNotifier(Peer *peer, Message *message)
{
    ...
    RPCEngine *e = RPCEngine::engine();
    QMetaObject::invokeMethod(e, "emitPuppetNotifierHelper", Qt::QueuedConnection,
                              Q_ARG(quint64, msg->puppetPtr),
                              Q_ARG(int, msg->signalIdx),
                              Q_ARG(int, msg->propertyIndex),
                              Q_ARG(QVariant, msg->propertyValue));
}
