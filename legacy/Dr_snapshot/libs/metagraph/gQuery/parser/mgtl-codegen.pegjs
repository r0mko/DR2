{
  ...
  function buildText(arg) 
  {
    var ret = []
    if (arg instanceof Array) 
      arg.forEach( function(e) { ret.push( buildText(e) ) } ) 
    else if (arg instanceof Object) {
      if ("value" in arg)
        ret.push(buildText(arg.value))
  ...
  function isQueryId(id) {
    if (typeof resolveQueryById == "function")
      return resolveQueryById(id) !== null
    return /m_/.test(id[0])
  }
  ...
  function localPathReference(id) {
    return { type: "PathReference", value: "path(" + id + ")", output: namedPaths[id].output, input: namedPaths[id].input }
  }
...
Identifier
  = !(ReservedWord/PathId) name:IdentifierName { return name; }
...
IdentifierStart
  = UnicodeLetter
  / "_"
  / "\\" sequence:UnicodeEscapeSequence { return text(); }
...
ThisContext
  = T_THIS { 
    return {
      type: "ThisContext",
      // token: buildTokenMeta(peg$currPos, text(), "PathId") 
    }
  }
...
PathElement
  = e:(PathId / LabeledPath / BracketedPath / NodeStatement / EdgeStatement) post:(__ PostFilterActions)? 
  { 
    lastElement = e
    if (e.output)
      ctxType = e.output
...
Traversal
  = TraversalNE { ctxType = "E"; return { type: "Traversal", direction: "NE", input: "N", output: "E" } }
  / TraversalEN { ctxType = "N"; return { type: "Traversal", direction: "EN", input: "E",  output: "N" } }
...
TraversalNE  "node -> edge[] traversal"
  ...
  / d: DirectionToken &( __ EdgeStatement) 
  / d:DirectionToken  &{ return ctxType === "N" }  { return d; }

TraversalEN "edge -> node traversal" 
  = d: DirectionToken &(__ NodeStatement) 
  / &(TraversalNE) d: DirectionToken
  / &(EdgeStatement __) d: DirectionToken
  / d:DirectionToken &{ return ctxType === "E" } { return d; }
...
PathId "path Id"
  = id:$IdentifierName &{ return isPathId(id) } { return localPathReference(id) }
  / q:QueryId "." id:$IdentifierName { return q + "." + 'path("' + id + '")' }

QueryId "query Id"
  = id: $IdentifierName &{ return isQueryId(id) } { return id }
...
ExecQuery
  = p:(PathId / BracketedPath) __ "(" _ arg:Context? _ ")" { 
    return {
      type: "EvalQuery",
      path: p,
      context: arg,
      output: p.output ? p.output : null
    }
  } 
...
LabelDef
  = id: $IdentifierName { return buildTokenMeta(peg$currPos, id, "Path") }
...
__
  = t:(WhiteSpace / t:LineTerminatorSequence / Comment )* { return t.join("") }
...
PropertyAssignment
  ...
  / T_GET __ key:PropertyName __
    "(" __ ")" __
    "{" __ body:FunctionBody __ "}"
  / T_SET __ key:PropertyName __
...
MemberExpression
  = first:(
        PrimaryExpression 
      / FunctionExpression
      / T_NEW __ callee:MemberExpression __ args:Arguments {
...
ObjectType
  = t: $IdentifierName &{ return checkType(t); } { return { type: "ObjectType", value: t } }
...
VariableStatement
  = t: $T_VAR __ declarations:VariableDeclarationList EOS { return [t, declarations, "\n"] } 
...
VariableDeclaration
  = id:$Identifier init:(__ Initialiser)? {
      var v = extractOptional(init, 1)
      var s 
      if (v.type == "Path") {
        defPathId(id, v)
        s = localPathReference(id)
      }
      return [id, "=", v ? (s ? s : v) : ""]
      // return id + v ? (s ? s : v) : ""
...
ReturnStatement
  ...
  / t: $T_RETURN _ argument:Expression EOS {
      var s 
      if (argument.type == 'Path') {
        defPathId('ret', argument)
        s = localPathReference('ret')
      } else 
        s = argument
      return [ t, s, "\n" ]
    }
...
ThrowStatement
  = $T_THROW _ argument:Expression EOS 

TryStatement
  = $T_TRY __ block:Block __ handler:Catch __ finalizer:Finally 
  / $T_TRY __ block:Block __ handler:Catch 
  / $T_TRY __ block:Block __ finalizer:Finally

Catch
  = $T_CATCH __ "(" __ param:Identifier __ ")" __ body:Block 

Finally
  4= T_FINALLY __ block:Block
