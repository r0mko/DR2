...
public:
    ...
    QSet<Edge*> dumpEdges(const QModelIndex &parent = QModelIndex()) const;
    QSet<Edge *> dumpEdges(const Node *parentNode) const;
    ...
    Node *importModelFile(const QUrl &source, const QModelIndex &parent = QModelIndex(), QString nodeId = QString());
    ...
    uint addEdge(const QModelIndex &srcNode, const QModelIndex &destNode, QString tag, QVariant data = QVariant());
...
public slots:
    ...
    bool loadModel(const QUrl &filename);
    ...
    uint addEdge(const NodeHandle &srcNode, const NodeHandle &destNode, QString tag, QVariant data = QVariant());
#ifdef QT_DEBUG
...
signals:
    ...
    void outEdgeAdded(const QModelIndex &index, EdgeHandle edgeHandle);
    void inEdgeAdded(const QModelIndex &index, EdgeHandle edgeHandle);
    void edgeDataChanged(Edge* edge);
    void edgeTagsChanged(Edge* edge);
...
private:
    ...
    typedef QHash<const Node*, QPointer<ModelAgent>> Node2Agent;                         ///< \c typedef для QHash<Node*, QPointer<ModelAgent>>

    NodeHandle *handleForIndex(const QModelIndex &index, int handleMetaTypeId) const;

    Node            *_root = nullptr;   ///< Корневой узел
    mutable QSet<Edge*>     edgeset;            ///< Временный список рёбер. Используется для различных внутренних нужд (загрузка/сохранение/подсчёт рёбер).
...
//    QList<QObject*> m_data;
    ...
    Node *importV2(QDataStream &in, const QModelIndex &parent, Node *parentNode);
