...
{
    chkItem(0);
    int extraflags = m_item->data(VisualInstance::ExtraFlagsKey).toInt();
    return (int)m_item->flags() | extraflags;
}
...
//        event->setAccepted(false);
    ...
    if (engine) {
        ...
        QJSValue v = engine->toScriptValue(e);
        QJSValue thisObj = engine->toScriptValue(handle);
        QJSValueList args;
        args.append(v);
        QJSValue r = handler.callWithInstance(thisObj, args);
        if (r.isError()) {
...
void VisualInstance::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    qDebug() << this << "dblcllick" << event->pos() << contains(event->pos()) << scene()->mouseGrabberItem();
...
{
//    qDebug() << this << "pressevent" << event->pos() << contains(event->pos()) << scene()->mouseGrabberItem();
...
void VisualInstance::preparePainter(QPainter *painter) const
{
...
QRectF VisualInstance::fixRect(const QRectF &rect) const
{
    int allFlags = (int)flags() | data(VisualInstance::ExtraFlagsKey).toInt();
    if (allFlags & VisualInstanceHandle::ItemIgnoresTransformations) {
        QTransform tr = currentViewTransform();
        qDebug() << "map rect" << rect << "to" << tr.mapRect(rect);
        return tr.mapRect(rect);
    }
    return rect;
}

QPainterPath VisualInstance::fixPath(const QPainterPath &path) const
{
    int allFlags = (int)flags() | data(VisualInstance::ExtraFlagsKey).toInt();
    if (allFlags & VisualInstanceHandle::ItemIgnoresTransformations) {
        QTransform tr = currentViewTransform();
