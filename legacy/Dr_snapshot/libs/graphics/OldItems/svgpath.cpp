...
void SvgParser::pathArc(QPainterPath &path,
                    qreal               rx,
                    qreal               ry,
...
bool SvgParser::parsePathDataFast(const QString &dataStr, QPainterPath &path)
{
...
QString SvgPath::svgPath() const
{
    return m_svgPath;
}

void SvgPath::setSvgPath(QString arg)
{
    if (m_svgPath == arg)
        return;

    m_svgPath = arg;
    emit svgPathChanged(arg);
    QPainterPath p;
    SvgParser::parsePathDataFast( arg, p );
    setPath( p );
}
