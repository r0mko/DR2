...
//    qDebug() << m_types;
    ...
    connect(this, &RPCEngine::writeProperty, this, &RPCEngine::writePropertyHelper, Qt::QueuedConnection);
    connect(this, &RPCEngine::emitPuppetNotifier, this, &RPCEngine::emitPuppetNotifierHelper, Qt::QueuedConnection);
    connect(this, &RPCEngine::emitPuppetSignal, this, &RPCEngine::emitPuppetSignalHelper, Qt::QueuedConnection);
}
...
void RPCEngine::setNetworkMessenger(NetMessenger *arg)
{
    if (m_net != arg) {
        ...
        m_net->registerMessageHandler(ObjectUnregistered, RPCEngine::h_ObjectUnregistered);
...
bool RPCEngine::registerGadget(QVariant gadget, QString id, bool notify)
{
    const QMetaObject *mo = QMetaType::metaObjectForType(gadget.userType());
    if (!mo) {
        qWarning() << Q_FUNC_INFO << "Failed to register gadget:" << gadget << "is not a Gadget type";
        return false;
    }
    if (id.isEmpty()) {
        id = QString("%1_%2(%3)").arg(mo->className()).arg(m_nodeName).arg(QString::number((qintptr)gadget.constData(), 16));
    }
    if (m_registeredGadgets.contains(id)) {
        qWarning() << qPrintable(QString("Gadget id %1 already registered!").arg(id));
        return false;
    }
    m_registeredGadgets.insert(id, gadget);
    if (!notify)
        return true;
    MSG_BODY(ObjectRegistered) b;
    b.objectName = id;
    b.isGadget = true;
    b.className = mo->className();
...
{
//    qDebug() << "registering object" << object << object->metaObject()->propertyCount();
    QString id;
    ...
    if (m_registeredObjects.contains(id)) {
        qWarning() << qPrintable(QString("QObject id %1 already registered!").arg(id));
        return false;
    }
...
//    for (auto it =
    ...
    //    }
    int i = 0;
    for (QPointer<QObject> o:m_registeredObjects) {

        if (o.isNull()) {
            qWarning() << "dead object at" << i;
            continue;
        }
...
bool RPCEngine::importType(RPCPeer *peer, int remoteTypeId, const QByteArray &name)
{
    int localTypeId = QMetaType::type(name.constData());
    if (localTypeId != QMetaType::UnknownType) {
        peer->addTypeMapping(localTypeId, remoteTypeId);
...
void RPCEngine::emitPuppetSignalHelper(quint64 puppetPtr, int signalIndex, QVariantList arguments)
{
    ...
    qDebug() << "emit signal at" << (void*)puppetPtr << arguments;
...
void RPCEngine::writePropertyHelper(quint64 objPtr, int propertyIndex, QVariantStreamer value)
{
    QObject *obj = reinterpret_cast<QObject *>(objPtr);
    QMetaProperty p = obj->metaObject()->property(propertyIndex);
    bool result = p.write(obj, value.reconstruct());
        qDebug() << "Updated property" << p.name() << "result" << result
                 << "\ntarget thread" << obj->thread()
