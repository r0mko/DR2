...
RPCPeer::~RPCPeer()
{
    qDebug() << this << "deleted";
    if (m_signalProxy) {
        delete m_signalProxy;
        m_signalProxy = nullptr;
    }
...
void RPCPeer::setPeer(Peer *peer)
{
    if (m_peer)
...
#ifdef QT_DEBUG
    if (remote != local) {
        qDebug() << __func__ << qPrintable(QString("info: type %1 (%2) has different remote id %3").arg(QMetaType::typeName(local)).arg(local).arg(remote));
    }
#endif
}
