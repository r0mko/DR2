...
VertexBuffer QTriangulator::triangulate()
{
    //    for (int i = 0; i < m_vertices.size(); ++i) {
    //        Q_ASSERT(qAbs(m_vertices.at(i).x) < (1 << 25));
    //        Q_ASSERT(qAbs(m_vertices.at(i).y) < (1 << 25));
    //    }
    ...
    VertexBuffer result;
    result.fillIndicesRef() = m_indices;
    result.verticesRef().resize(m_vertices.size());
    for (int i = 0; i < m_vertices.size(); ++i) {
        QPointF p(m_vertices.at(i).x, m_vertices.at(i).y);
        p /= Q_FIXED_POINT_SCALE;
        result.verticesRef()[i] = p;
    }
    return result;
}
...
void QTriangulator::initialize(const qreal *polygon, int count, uint hint, const QTransform &matrix)
{
    ...
    m_indices[count] = quint32(-1); //Q_TRIANGULATE_END_OF_POLYGON
}
        ...
        for (int i = 0; i < path.elementCount(); ++i, ++e, p += 2) {
            ...
            case QPainterPath::MoveToElement:
                if (!m_indices.isEmpty())
                    m_indices.push_back(quint32(-1)); // Q_TRIANGULATE_END_OF_POLYGON
                // Fall through.
...
void QTriangulator::ComplexToSimple::decompose()
{
    ...
    for (int first = 0; first < m_edges.size(); ++first) {
        ...
        m_parent->m_indices.push_back(quint32(-1)); // Q_TRIANGULATE_END_OF_POLYGON
    }
}
...
void QTriangulator::ComplexToSimple::initEdges()
{
    ...
    for (int i = 0; i < m_parent->m_indices.size(); ++i) {
        if (m_parent->m_indices.at(i) == quint32(-1)) { // Q_TRIANGULATE_END_OF_POLYGON
...
void QTriangulator::SimpleToMonotone::decompose()
{
    ...
    for (int first = 0; first < m_edges.size(); ++first) {
        ...
        if (m_parent->m_indices.size() > 0 && m_parent->m_indices.back() != quint32(-1)) // Q_TRIANGULATE_END_OF_POLYGON
            m_parent->m_indices.push_back(quint32(-1)); // Q_TRIANGULATE_END_OF_POLYGON
    }
}
...
void QTriangulator::SimpleToMonotone::setupDataStructures()
{
    ...
    while (i + 3 <= m_parent->m_indices.size()) {
        ...
        } while (m_parent->m_indices.at(i) != quint32(-1)); // Q_TRIANGULATE_END_OF_POLYGON
...
void QTriangulator::MonotoneToTriangles::decompose()
{
    ...
    while (m_first + 3 <= m_parent->m_indices.size()) {
        m_length = 0;
        while (m_parent->m_indices.at(m_first + m_length) != quint32(-1)) { // Q_TRIANGULATE_END_OF_POLYGON
            ++m_length;
...
VertexBuffer triangulate(const QPainterPath &path, const QTransform &matrix, qreal lod)
{
