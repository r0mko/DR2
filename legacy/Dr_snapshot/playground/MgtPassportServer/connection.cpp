...
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] destroyed";
    emit finished();
}
...
//    qDebug() << "boundValues" << q.boundValues();
    q.exec();
    emit activity( QDateTime::currentMSecsSinceEpoch() );
    if( q.lastError().type() != QSqlError::NoError ) qDebug() << " ===>> SQL ERROR" << q.lastError().text();
    int lii = q.lastInsertId().toInt();
//    qDebug() << "last insert id is:" << lii;
    return lii;
...
void Connection::onDataManipulation(Connection *c, QVariantMap m)
{
//    if (this == c) return; // TODO: remove all updates in Table.qml and remove this line
    QVariantMap msg;
    msg.insert( "messageType", "updateNotice" );
    msg.insert( "dataSource", m.value("dataSource").toString() );
    msg.insert( "action", m.value("method").toString() );
    msg.insert( "initiatorMessage", m );
    msg.insert( "initiatorId", c->id() );

    sendMessage( msg ); // TODO: create and send message informing of changes
...
void Connection::processMessage(const QVariantMap &msg)
{
    if( !msg.contains( "messageType" ) ) {
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] processing message skipped, no messageType defined";
        QVariantMap m;
...
//    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] processing message type" << msg.value( "messageType" ).toString();
    ...
    else qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] unknown message type";



    if( messageType == QString( "dataAdd" ) || messageType == QString( "dataDel" ) || messageType == QString( "dataEdit" ) )
        emit dataManipulation( this, msg );
...
//    if( !checkDatabase() ) return;
    ...
    if( messageType != QString( "login" ) ){
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] unauthorized client request rejected";
        QVariantMap m;
        m.insert( "messageType", "error" );
        m.insert( "replyId", msg.value( "messgeId" ).toInt() );
    ...
    if( m_authorized ){
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] already authorized client asked for authorizetion again!";
        QVariantMap m;
        m.insert( "messageType", "error" );
        m.insert( "replyId", msg.value( "messgeId" ).toInt() );
    ...
    if( !m_database->isOpen() || query.lastError().type() != QSqlError::NoError ){
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] database error: " << query.lastError().text();
        m_database->close();

        /* retry */
        emit loginRetry( msg );
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] retrying";
        /**/
...
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login id: " << m_userId;
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login name: " << m_userName;
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login role: " << m_roleName;
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login can read: " << m_canRead;
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login can write: " << m_canWrite;
//        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] user login can manage users: " << m_canManageUsers;
...
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] Hello.";
    QVariantMap m;
...
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] User logout";
...
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] Client asked server status";
    QVariantMap m;
...
void Connection::processStopServerMessage(const QVariantMap &msg)
{
    ...
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] unknown message type";
}
...
bool Connection::checkDatabase()
{
    if( !m_server->databaseOpened() ) {
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] we lost database connection";
...
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] connected.";
}

void Connection::onDisconnected()
{
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] disconnected";
}

void Connection::onError( QAbstractSocket::SocketError error )
{
    if( error != QAbstractSocket::RemoteHostClosedError )
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] error" << error;
}
...
void Connection::onStateChanged( QAbstractSocket::SocketState state )
{
    if( state != QAbstractSocket::ClosingState && state != QAbstractSocket::UnconnectedState )
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] state changed to" << state;
}
...
void Connection::onReadyRead()
{
    ...
    if( m_blockSize == 0 ) {
        ...
        if( m != magic ) {
            ...
            qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] recieved garbage. skipping";
...
#ifndef NONETDEBUGOUTPUT
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] <<== recieved a message" << m_socket->bytesAvailable() << "bytes";
#endif
...
void Connection::start()
{
    ...
    if (!m_socket->setSocketDescriptor(m_handle) ) {
        qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] couldn't establish connection to peer.";
        emit finished();
        return;
    }
    ...
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] connection created and established";
...
void Connection::sendMessage(QVariantMap msg)
{
    ...
    out.device()->seek(0);
#ifndef NONETDEBUGOUTPUT
    qDebug() << QDateTime::currentDateTime().toString( "[ dd/MM/yyyy hh:mm:ss.zzz ]" ).toStdString().c_str() << "[ Connection" << m_handle << "] ==>> sending a message" << block.size() << "bytes";
#endif
