...
GuideView {
    ...
    EdgeModel {
        ...
        function buildSegment(h, index) {
            var ed = h.getData()
            console.log("building segment %1 from edge uid %2".arg(index).arg(h.uid), h.startNode, h.endNode)
            var seg = addSegment(index, ed.guide, ed.color, 3)
            seg.offset = Math.round(Math.random() * 5)
        }
    }
}
