...
signals:
    ...
    void dataManipulation( Connection*, QVariantMap );
    void msgChanged(QVariantMap arg);
    void activity(qint64 timestamp);

public slots:
    void onDataManipulation(Connection*c, QVariantMap m); // called by server
    inline int id() const { return m_handle; }
