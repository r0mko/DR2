    ...
    menuBar: MenuBar {
        ...
        Menu {
            ...
            MenuItem {
                text: 'keyss'
                onTriggered: {
                    for( var k in myeNNNdgeModel.keyss )
                    console.log('types', k)
                }
            }
        }
                ...
                Scene {
                    ...
                    Loader {
                        ...
                        sourceComponent: GeoView {
                            ...
                            onViewTransformChanged: console.log("view transform", viewTransform)
                        }
                    }
                }
...
//                outline: true
    ...
    Action {
        ...
        onTriggered: {
            saveFileselector.modality = Qt.WindowModal
...
//                modality: Qt.NonModal
    ...
    RPCEngine {
        id: engine
        nodeName: '<SPA>'
        property variant marker: null
        property variant markerObj: null
        networkMessenger {
            id: netman
            //            serverPort: 42033
            listening: true
        ...
        onObjectRegistered: {
            ...
            if (name == "poster") {
                marker = Qt.createComponent("Marker.qml");
                if (marker.status == Component.Ready) {
                    var poster = engine.queryRemoteObject(peer, name)
                    console.log("Object", poster, poster.posX, poster.posY, poster.text)
                    markerObj = marker.createObject(m_scene, {color: "red", x: poster.posX, y: poster.posY, text: poster.text, rpcObject: poster, rpcObjectName: name, rpcObjectName: className });
                    if (markerObj == null)
                        console.log("Error creating Marker object");
                }
            }
        }

        onObjectUnRegistered: {
            console.log( 'Object unRegistered', peer, name )
            if (name == "poster")
                markerObj.destroy()
        }
...
//                console.log( 'Count POLYS', textLayer.count, count, attributes.name_2 )
                if( keyss === null ) keyss = {}
                keyss[ attributes.type_2 ] = attributes.type_2
            }
    ...
    GraphModel {
        ...
        NodeChildrenModel {
            ...
            onCountChanged: { console.log("rebuild rpcWays"); rpcWays.build() }
            function createWays() {
                if (!linked) {
            ...
            function buildWay(index) {
                ...
                i.pen.cosmetic = true
                i.pen.strokeStyle = h.metadata.repairs ? 3 : 1
            ...
            function updateWay(index) {
                ...
                i.pen.color = h.metadata.color
                console.log("Current repairs value", h.metadata.repairs, index)
                i.pen.strokeStyle = h.metadata.repairs ? 3 : 1
                var guide = creatist.deserialize("Guide", h.data)
        ...
        QtObject {
            id: rpcWays
            objectName: "Ways"
            property variant private_agents: []
            property variant ways: []

            function build() {
                for (var i = 0; i < ways_model.count; ++i) {
                    var h = ways_model.getHandle(i)
                    var ag = h.agent
                    engine.registerObject( ag, true )

                    private_agents.push(ag)

                    if (ways[h.metadata.idf] === undefined )
                        ways[h.metadata.idf] = []
                    ways[h.metadata.idf].push(h.agent)
                    console.log("Init ways", h.metadata.idf)
                }
            }

            Component.onCompleted: {
                console.log("Init rpcWays")
                build()
            }
        }
        ...
        function init() {
            ...
            fNode.create("cache")
            fNode.create("ways")
            engine.registerObject( rpcWays, true )
        }
    }
    ...
    EdgeModel {
        ...
        function onHoverEnter(event) {
            ...
            hover_handle.runner = this.result.findNearestPos(event.pos)
            hover_handle.visible = true
            hover_handle.updateHandle(this.result.pathData())
        }
        function onHoverMove(event) {
            ...
            for (var i = 0; i < count; ++i) {
                ...
                if (i != 0 && i != count - 1) {
                    if (s.startPos > s.endPos)
                        path.appendGuide(item.source.reversed())
                    else
                        path.appendGuide(item.source)
                } else {
                    var partPath = item.source.part(s.startPos, s.endPos)
                    path.appendGuide(partPath)
                }
                fBlob.metadata = {
                    edgeUid: s.uid,
                    startPos: s.startPos,
...
//        onGotMessage: {
//            console.log( message )
//            var regexp = /DAT\s\d\s(\d+)\s\S+\s(0x\S+)\s(\S+)\s(\d+\.\d+)/
