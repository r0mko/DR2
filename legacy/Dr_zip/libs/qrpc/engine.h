...
public:
    ...
    inline QHash<Peer *, RPCPeer *> peers() const { return m_peers; }
...
signals:
    ...
    void gadgetRegistered(RPCPeer *peer, QString name, QString className);

    void objectUnRegistered(RPCPeer *peer, QString name);

    /* "method" signals */
    void emitPuppetSignal(quint64 puppetPtr, int signalIndex, QVariantList arguments);
    void emitPuppetNotifier(quint64 puppetPtr, int signalIndex, int propertyIndex, QVariant value);
    void writeProperty(quint64 objPtr, int propertyIndex, QVariantStreamer value);


public slots:
    void setNodeName(QString arg);
    void setNetworkMessenger(NetMessenger* arg);
    bool registerGadget(QVariant gadget, QString id, bool notify = false);
...
private slots:
    ...
    void writePropertyHelper(quint64 objPtr, int propertyIndex, QVariantStreamer value);
...
private:
    ...
    QHash<QString, QPointer<QObject>> m_registeredObjects;
    QHash<QString, QVariant> m_registeredGadgets;
