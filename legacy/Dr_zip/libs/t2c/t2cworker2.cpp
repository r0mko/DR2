...
void T2CWorker2::disconnectFromT2C()
{
    if (m_socket == nullptr) {
        qWarning() << "somehow socket is nullptr";
        return;
...
void T2CWorker2::parse(const QByteArray &line)
{
    if( !m_t2cok && line == "04.01" ) {
        m_t2cok = true;
        qDebug() << "T2C alive";
        for (auto idf: m_queue) {
            if( !m_subscribed.contains(idf) )
                sendCommand(cmdListen(idf));
            m_subscribed.insert(idf);
        }
        m_queue.clear();
    }
...
void T2CWorker2::onSocketStateChanged(QAbstractSocket::SocketState socketState)
{
    ...
    if( m_started ) {
        if (socketState == QAbstractSocket::UnconnectedState)
            QTimer::singleShot(500, this, &T2CWorker2::connectToT2C);
        else
...
{
    if (m_t2cok && !m_subscribed.contains(idf)) {
        m_subscribed.insert(idf);
        sendCommand(cmdListen(idf));
    }else if(!m_queue.contains(idf)){
        m_queue.insert(idf);
    }
}

void T2CWorker2::sendMessage(QString msg)
{
    sendCommand(msg.toLocal8Bit());
}

void T2CWorker2::setValue(int idf, QString type, QVariant value)
{
    sendCommand(QString("DAT 0 %1 %2 %3\n").arg(idf).arg(type).arg(value.toString()).toLocal8Bit());
}
