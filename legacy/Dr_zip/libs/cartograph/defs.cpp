...
//                    break;
    ...
    for( uint ii = 0; ii < (uint)vhits.size(); ++ii ) {
        if( vhits[ii] == 0 ) continue; // done
        if( vhits[ii] == 2 ) continue; // will get there
        if( vhits[ii] == 1 ) { // path pegins
            QPolygonF p;
            auto nextSet = adjacencyHash[ii];
            if( nextSet.size() > 1 ) qFatal("FATAL");
            auto prev = ii;
            auto next = *nextSet.begin();


            adjacencyHash.remove( ii );
            p.append( m_vertices.at( ii ) );
            vhits[ii] = 0;
            forever {
                auto nextSet = adjacencyHash[next];
                auto setr = nextSet.begin();
                if( *setr == prev ) setr++;
                if( setr == nextSet.end()) break;
                prev = next;
                next = *setr;
                p.append(m_vertices.at(prev));
                adjacencyHash.remove(prev);
                vhits[prev] = 0;
//                ii++;
            }
            adjacencyHash.remove(next);
            vhits[next] = 0;
            ret.append( p );
            qDebug() << "added 1";
        }
