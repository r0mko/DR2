...
#define METAGRAPH_NODE(Class)                                                \
    ...
    public:                                                                  \
    virtual inline int typeId() const;
...
//    QList< Node * >                         m_childNodes;   ///> дочерние узлы
    ...
    stable_vector<Node*>                    m_childNodes;   ///> дочерние узлы
    QList< QSharedPointer<Edge> >           m_outEdges;     ///> исходящие рёбра
    QList< QWeakPointer<Edge> >             m_inEdges;      ///> входящие рёбра
    mutable stable_vector<Node*>::const_iterator    it;             ///> индекс в родительском узле
//    mutable QList<Node*>::const_iterator    it;             ///> индекс в родительском узле
    ...
    inline const stable_vector<Node*> &childNodes() const { return m_childNodes; }
//    inline const QList<Node*> &childNodes() const { return m_childNodes; }
    ...
    bool isAncestorOf(const Node *other, bool onlyDirect = false) const;
    bool isChildOf(const Node *other, bool onlyDirect = false) const;

    //children getters
    inline int childCount() const { return m_childNodes.size(); }
    inline bool isValid() const { return it.node_pointer() != nullptr; }
//    inline bool isValid() const { return it.i != nullptr; }
    ...
    inline stable_vector<Node*> &_childNodes() { return m_childNodes; }
...
int Node::typeId() const { return SELF_TYPEID; }
...
public:
    ...
    Q_INVOKABLE QVariant findChild(QString id) const;
...
class NodeFactory : public QObject, public QQmlParserStatus
{
    ...
    Q_PROPERTY(NodeHandle parentHandle READ parentHandle WRITE setParentHandle NOTIFY parentHandleChanged RESET resetParent)
...
//QDataStream &operator <<(QDataStream &out, const QModelIndex &idx)
//{
//    out << idx.row() << idx.column() << (quint64)idx.internalPointer() << (quint64)idx.model();
//}

//QDataStream &operator >>(QDataStream &in, QModelIndex &idx)
//{
//    int row, column;
//    quint64 ptr;
//    quint64 model;
//    in >> row >> column >> ptr >> model;
//    idx = QModelIndex(row, column, (void*)ptr, (QAbstractItemModel*)model);
//}
