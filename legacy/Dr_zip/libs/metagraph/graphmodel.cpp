 ...
 */
bool GraphModel::loadModel(const QUrl &source)
{
    QElapsedTimer t;
    t.start();
    QString file = urlToFilename(source);
    if (QFile::exists(file)) {
        beginResetModel();
        mute = true;
        initModel();
        Node *r = importModelFile(source, QModelIndex(), "");
        mute = false;
        endResetModel();
        roleNames();
        if (r) {
            qDebug() << "model" << file << "imported in" << t.elapsed() << "ms";
            return true;
        }
    }
    return false;
 ...
 */
Node *GraphModel::importModelFile(const QUrl &source, const QModelIndex &parent, QString nodeId)
{
    QString fileId;
    ...
    if (magic != MAGIC_V2) {
        if (magic == MAGIC_V1)
            qWarning() << "Error loading" << f.fileName() << ": old file format";
        else
    ...
    Node *ret = importV2(in, parent, parentNode);
    if (in.status() != QDataStream::Ok) {
...
Node *GraphModel::importV2(QDataStream &in, const QModelIndex &parent, Node *parentNode)
{
    int count;
    in >> count;
    QElapsedTimer t;
    ...
    qDebug() << "imported tree of" << parentNode->treeSize() << "nodes in" << t.elapsed() << "ms";
    t.restart();
    //    newNode->fixIt(true);
    uint s = Edge::count;
    importEdges(in, newIdx);
    qDebug() << "imported" << Edge::count - s << "edges in" << t.elapsed() << "ms";
    if (!mute) {
    ...
    qDebug() << "fixed iterators in" << t.elapsed() << "ms";
...
//    }
    ...
    edge->setData(data);
    //    debugPrint << "adding edge" << edge->getUid() << "from" << getMRL(start) << "to" << getMRL(end) << "tags" << tags << "mute" << mute;
    ...
    if (!mute) {
        ...
        emit outEdgeAdded(start, EdgeHandle(edge.data(), this));
        emit inEdgeAdded(end, EdgeHandle(edge.data(), this));
    }
    return edge;
}
...
uint GraphModel::addEdge(const NodeHandle &srcNode, const NodeHandle &destNode, QString tag, QVariant data)
{
    //    qDebug() << "adding edge" << srcNode.nodeId() << destNode.nodeId();
    return addEdge(srcNode.modelIndex(), destNode.modelIndex(), tag, data);
}
...
uint GraphModel::addEdge(const QModelIndex &srcNode, const QModelIndex &destNode, QString tag, QVariant data)
{
...
QSet<Edge *> GraphModel::dumpEdges(const QModelIndex &parent) const
{
    Node *n = getNode(parent);
    return dumpEdges(n);
}

QSet<Edge *> GraphModel::dumpEdges(const Node *parentNode) const
{
    QSet<Edge *> ret;
    for (Node *n : parentNode->childNodes()) {
        for (QSharedPointer<Edge> e : n->outEdges())
            ret.insert(e.data());
        for (QWeakPointer<Edge> e : n->inEdges()) {
            auto strong = e.toStrongRef();
            if (strong)
                ret.insert(strong.data());
        }
        ret.unite(dumpEdges(n));
    }
    return ret;
}
/**
 ...
 *
 * Если узлу был присвоено имя, то оно заменяется на новое. Всем дочерним узлам рассылается уведомление об изменении их адреса.
 */
...
QList<Edge *> GraphModel::exportEdges(QDataStream &out, const QModelIndex &root) const
{
    ...
    out << MAGIC_V2;
    out << edgeset.size();
    qDebug() << __func__ << "export" << edgeset.size() << "edges";
    for (Edge *e : edgeset) {
        bool ok = true;
...
QModelIndexList GraphModel::importEdges(QDataStream &in, const QModelIndex &root)
{
    ...
    if (c == 0 && !in.atEnd()) {
        ...
        if (magic == MAGIC_V2) {
            in >> c;
            if (c > 0)
                qDebug() << "importing new edges format, count =" << c;
            oldFormat = false;
        } else {
        ...
        if (c > 0)
            qDebug() << "importing old edges format, count =" << c;
    }
    //    qDebug() << "importing" << c << "edges to root" << getMRL(root);
    uint maxUid = Edge::count;
//    QElapsedTimer t;
    QList<int> st;
    QList<int> end;
    QString tag;
...
//            t.restart();
            uint u = addEdge(src, dst, tag, data);
            if (u > maxUid) {
                maxUid = u;
            }
//            qDebug() << t.nsecsElapsed() << "nsecs to add edge";
...
bool GraphModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    //    qDebug() << "remove" << count << "rows starting from" << row << "in" << getMRL(parent) << "Child count" << n->childCount();
...
bool GraphModel::moveRows(const QModelIndex &sourceParent, int sourceRow, int count, const QModelIndex &destinationParent, int destinationRow)
{
    ...
    //    qDebug() << "moving" << count << "nodes from" << getMRL(sourceParent) << "at" << sourceRow << "to" << getMRL(destinationParent) << "at" << destinationRow << newParent->childCount() << mute;
    NId &old_pid = node2nid[oldParent];
    NId &new_pid = node2nid[newParent];
    ...
    if (oldParent != newParent) {
        ...
        if (destinationRow == newParent->childCount())
            newParent->m_childNodes.insert(newParent->m_childNodes.end(), nlist.begin(), nlist.end());
        else {
            auto it = newParent->m_childNodes.begin() + destinationRow;
            newParent->m_childNodes.insert(it, nlist.begin(), nlist.end());
        }
        newParent->fixIt();
        oldParent->fixIt();
        endInsertRows();
