  ...
  //    m_enabled( true )
{
    ...
    initSceneImpl(new QGraphicsScene(this));
...
void Scene::setViewport(QQuickItem *arg)
{
    ...
    m_viewport = arg;
    connect(m_viewport, SIGNAL(widthChanged()), this, SLOT(adjustView()), Qt::QueuedConnection);
    connect(m_viewport, SIGNAL(heightChanged()), this, SLOT(adjustView()) , Qt::QueuedConnection);
...
//    QSignalBlocker block(m_renderer);
    m_renderer->setWidth(visibleRect.width());
...
void Scene::initSceneImpl(QGraphicsScene *scene)
{
    m_scene = scene;
    ...
    m_scene->setUserData(0, new SceneData);
}
