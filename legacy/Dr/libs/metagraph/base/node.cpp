...
{
    removeChild(m_childNodes.indexOf(child));
}

void Node::removeChild(int n)
{
    if (n >= 0 && n < m_childNodes.size())
        delete m_childNodes.takeAt(n);
}
...
void Node::setParent(Node *parent)
{
    ...
    if (m_parent) {
        m_parent->m_childNodes.removeAll(this);
        m_parent->fixIt();
    }
    m_parent = parent;
}
...
//    qDebug() << "Moving" << count << "children from" << from << "to" << to;
    if (to < from) {
        ...
        while(count--)
            m_childNodes.move(last, to);
    } else if (to > from + count) {
        while (count--)
            m_childNodes.move(from, to - 1);
    }
    fixIt();
}
...
int MetaNode::typeId() const { return SELF_TYPEID; }
