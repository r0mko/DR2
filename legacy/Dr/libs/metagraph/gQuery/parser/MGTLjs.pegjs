{
  ...
  function defPathId(label, path) {
    if (label in namedPaths) {
      throw new Error("duplicate context id <"+ JSON.stringify(label) + ">!")
      return null
    }
    path.localId = Object.keys(namedPaths).length 
    namedPaths[label] = path
  }

  function defPath(path) {
    var l = Object.keys(namedPaths).length 
    var label = "_path#" + l 
    path.localId = l
    namedPaths[label] = path
    return label
  ...
  function isPathId(id) {
    var ret = id in namedPaths
    return ret
  }

  function isQueryId(id, offset) {
    if (knownIds[offset] == 2)
      return true;
    var ret
    if (typeof resolveQueryById == "function")
      ret = resolveQueryById(id) !== null
    else
      ret = id.indexOf('m_') == 0
    if (ret === true) {
      knownIds[offset] = 2
  ...
  function checkType(typeName, offset) {
    if (knownIds[offset] == 1)
      return true;
    var ret
    if (typeof isRegistered == "function")
      ret = isRegistered(typeName)
    else
      ret = typeName[0]>='A' && typeName[0]<='Z'
    if (ret === true) {
      knownIds[offset] = 1
    }
    return ret
  ...
  function checkReduceFn(funcName, offset) {
    // if (knownIds[offset] == 3)
    //   return true;
    var ret;
    if (typeof findReduceFn == "function")
      ret = findReduceFn(funcName)
    else
      ret = ["one", "last", "first"].indexOf(funcName) >= 0
    if (ret)
      knownIds[offset] = 3
    return ret
  }
  ...
  function buildPathRef(id) {
    return { 
      type: "PathReference", 
      id: id, 
      pathIndex: namedPaths[id].localId, 
      output: namedPaths[id].output, 
      input: namedPaths[id].input 
    }
  }

  function buildExecPath(pathId, argument) {
    return { 
      type: "ExecPath", 
      path: buildPathRef(pathId), 
      id: pathId,
      argument: argument 
    }
  }
  ...
  function buildPathInstance(id, offset) {
    var instIdx = 1
    if (pathInstances[id]) {
      instIdx = pathInstances[id].length + 1
      pathInstances[id].push(offset)
    } else {
      pathInstances[id] = []
    }
    var ret = buildPathRef(id)
    ret.type = "PathInstance"
    ret.instance = instIdx
    return ret
  }
}
Start
  = __ program:Program __ { program.metadata = tokenMeta; return program; }
...
NullLiteral
  = T_NULL { return { type: "Literal", value: null }; }

BooleanLiteral
  = T_TRUE  { return { type: "Literal", value: true  }; }
  / T_FALSE { return { type: "Literal", value: false }; }
...
Path
  = st: PathStart p: (__ t: Traversal t2: Traversal? __ c: PathElement { 
    if (t.output == "?") {
      if (t.type == "Traversal") {
        if (c.output == "E") {
          t.output = "N"
    ...
    return t2 ? [t, t2, c] : [t, c] 
  })* {
    var steps = []
    if (st && st.type !== "ThisContext")
      steps.push(st)
    steps.push(p)
    var etype = ctxType
    ctxType = ""
    var lst = flatten(steps)
    var intype = "?"
    if (lst.length) {
      intype = lst[0].input ? lst[0].input : (lst[1] ? lst[1].input ? lst[1].input : "?" : "?")
    }
    return {
      type: "Path",
      input: intype,
...
PathStart
  = e:(PathElement / Context) { lastElement = e; if (e.output) ctxType = e.output; else ctxType = "?"; return e}

ThisContext
  = T_THIS { 
    buildTokenMeta(peg$currPos, text(), "ThisContext") 
    return {
      type: "ThisContext"
    }
  }
...
Context
  = ctx:(ExecQuery / ThisContext) { return { type: "Context", value: ctx } }

ContextProperty
  = ctx:Context __ "." __ prop: (CallExpression / MemberExpression) { 
    return {
...
PathElement
  = e:(PathInstance / LabeledPath / BracketedPath / NodeStatement / EdgeStatement) post:(__ PostFilterActions)? 
  { 
    lastElement = e
    if (e.output)
      ctxType = e.output
    e.post = extractOptional(post,1); 
    return e
  }

PathInstance
  = id: $PathId {
    return buildPathInstance(id, peg$reportedPos)
  }

Traversal
  = TraversalNE { ctxType = "E"; return { type: "Traversal", direction: "NE", input: "N", output: "E", kind: text() } }
  / TraversalEN { ctxType = "N"; return { type: "Traversal", direction: "EN", input: "E",  output: "N", kind: text() } }
  / ChildrenTraversal { ctxType = "N"; return { type: "ChildrenTraversal", input: "N",  output: "N" } }
  / ParentTraversal { ctxType = "N"; return { type: "ParentTraversal", input: "N", output: "N" } }
  / NullTraversal { return { type: "Skip", input: ctxType, output: ctxType } }
  / Recursion { return { type: "Recursion", input: ctxType, output: ctxType } }
  / FallbackTraversal 


TraversalNE  "node -> edge[] traversal"
  = $DirectionToken &(TraversalEN) 
  / $DirectionToken &( __ EdgeStatement) 
  / $DirectionToken  &{ return ctxType === "N" }  

TraversalEN "edge -> node traversal" 
  = $DirectionToken &(__ NodeStatement) 
  / &(TraversalNE) $DirectionToken
  / &(EdgeStatement __) $DirectionToken
  / $DirectionToken &{ return ctxType === "E" } 


FallbackTraversal "fallback traversal" 
  = d: DirectionToken !DirectionToken { return { type: "Traversal", direction: "?", input: "?", output: "?" } }
...
PathId "path Id"
  = id:$IdentifierName &{ return isPathId(id) } { return { type: "PathId", pathId: id } }
  / q:QueryId __ "." __ id:$IdentifierName { return { type: "ForeignPathId", query: q, pathId: id} }

QueryId "query Id"
  = id: $IdentifierName &{ return isQueryId(id, offset() ) } { return id }

NodeStatement 
  = l: StatementLabel? _ f: NodeFilter
  {
    return {
      type: "NodeStatement",
      label: l,
...
ExecQuery
  = p:PathId __ "(" _ arg:Context? _ ")" { 
    return buildExecPath(p.pathId, arg)
  } 

LabeledPath
  = id: LabelDef q: BracketedPath { return { id: id, path: q } }
...
LabelDef
  = id: $IdentifierName { buildTokenMeta(peg$currPos, id, "PathId"); return id }

BracketedPath
  = "(" __ q: Path __ ")" { return q }

NodeFilter
  = "(" __ l: LogicalORExpression __ ")" { return l }
...
ReduceFnName "reduce function name"
  = id: Identifier &{ return checkReduceFn(id, offset()) } { return id; }
...
CaptureRoutine "CaptureRoutine"
  ...
  / "+" c:SubstitutedCaptureList { return c; }

ContextCapture
  = ctx:PathId __ "+=" __ c: (SingleCapture / SubstitutedCaptureList) {
    return {
      type: "ContextCapture",
      context: ctx,
      capture: c
...
T_BREAK      = "break"      !IdentifierPart
T_CASE       = "case"       !IdentifierPart
T_CATCH      = "catch"      !IdentifierPart
T_CLASS      = "class"      !IdentifierPart
T_CONST      = "const"      !IdentifierPart
T_CONTINUE   = "continue"   !IdentifierPart
T_DEBUGGER   = "debugger"   !IdentifierPart
T_DEFAULT    = "default"    !IdentifierPart
T_DELETE     = "delete"     !IdentifierPart
T_DO         = "do"         !IdentifierPart
T_ELSE       = "else"       !IdentifierPart
T_ENUM       = "enum"       !IdentifierPart
T_EXPORT     = "export"     !IdentifierPart
T_EXTENDS    = "extends"    !IdentifierPart
T_FALSE      = "false"      !IdentifierPart
T_FINALLY    = "finally"    !IdentifierPart
T_FOR        = "for"        !IdentifierPart
T_FUNCTION   = "function"   !IdentifierPart
T_GET        = "get"        !IdentifierPart
T_IF         = "if"         !IdentifierPart
T_IMPORT     = "import"     !IdentifierPart
T_INSTANCEOF = "instanceof" !IdentifierPart
T_IN         = "in"         !IdentifierPart
T_NEW        = "new"        !IdentifierPart
T_NULL       = "null"       !IdentifierPart
T_RETURN     = "return"     !IdentifierPart
T_SET        = "set"        !IdentifierPart
T_SUPER      = "super"      !IdentifierPart
T_SWITCH     = "switch"     !IdentifierPart
T_THIS       = "this"       !IdentifierPart
T_THROW      = "throw"      !IdentifierPart
T_TRUE       = "true"       !IdentifierPart
T_TRY        = "try"        !IdentifierPart
T_TYPEOF     = "typeof"     !IdentifierPart
T_VAR        = "var"        !IdentifierPart
T_VOID       = "void"       !IdentifierPart
T_WHILE      = "while"      !IdentifierPart
T_WITH       = "with"       !IdentifierPart
...
MemberExpression
  = first:(
        PrimaryExpression 
      / FunctionExpression
      / T_NEW __ callee_expr:MemberExpression __ args:Arguments {
          return { type: "NewExpression", callee_expr: callee_expr, arguments: args };
        }
    )
    rest:(
...
NewExpression
  = MemberExpression
  / T_NEW __ callee_expr:NewExpression {
      return { type: "NewExpression", callee_expr: callee_expr, arguments: [] };
    }

CallExpression
  = first:(
      callee_expr:MemberExpression __ args:Arguments {
        return { type: "CallExpression", callee_expr: callee_expr, arguments: args };
      }
    )
    rest:(
        __ args:Arguments {
...
ObjectType
  = t: $IdentifierName &{ return checkType(t, offset()); } { buildTokenMeta(peg$currPos, t, "ObjectType"); return { type: "ObjectType", value: t } }
...
VariableStatement
  = T_VAR __ declarations:VariableDeclarationList EOS {
      return {
...
VariableDeclaration
  = id:$LabelDef __ "=" !"=" __ path:Path {
      defPathId(id, path)
      return {
        type: "VariableDeclarator",
        id:   id,
        init: buildPathRef(id)
      };
    }
...
ExpressionStatement
  = !("{" / T_FUNCTION) expression:Expression EOS {
      return {
...
IfStatement
  = T_IF __ "(" __ test:Expression __ ")" __
...
IterationStatement
  = T_DO __
    body:Statement __
    T_WHILE __ "(" __ test:Expression __ ")" EOS
    { return { type: "DoWhileStatement", body: body, test: test }; }
  / T_WHILE __ "(" __ test:Expression __ ")" __
    body:Statement
  ...
  / T_FOR __
    "(" __
    T_VAR __ declarations:VariableDeclarationListNoIn __ ";" __
    test:(Expression __)? ";" __
    update:(Expression __)?
    ")" __
    body:Statement
  ...
  / T_FOR __
    "(" __
    T_VAR __ declarations:VariableDeclarationListNoIn __
    T_IN __
    right:Expression __
    ")" __
    body:Statement
...
ContinueStatement
  ...
  / T_CONTINUE _ label:Identifier EOS {
...
BreakStatement
  ...
  / T_BREAK _ label:Identifier EOS {
...
ReturnStatement
  ...
  / T_RETURN _ argument:Expression EOS {

      return { type: "ReturnStatement", argument: argument };
    }

WithStatement
  = T_WITH __ "(" __ object:Expression __ ")" __
    body:Statement
    { return { type: "WithStatement", object: object, body: body }; }

SwitchStatement
  = T_SWITCH __ "(" __ discriminant:Expression __ ")" __
    cases:CaseBlock
    {
      return {
...
CaseBlock
  = "{" __ clauses:(CasePathElements __)? "}" {
      return optionalList(extractOptional(clauses, 0));
    }
  / "{" __
    before:(CasePathElements __)?
    default_:DefaultPathElement __
    after:(CasePathElements __)? "}"
    {
...
CasePathElements
  = first:CasePathElement rest:(__ CasePathElement)* { return buildList(first, rest, 1); }

CasePathElement
  = T_CASE __ test:Expression __ ":" consequent:(__ StatementList)? {
      return {
        type:       "SwitchCase",
        test:       test,
...
DefaultPathElement
  = T_DEFAULT __ ":" consequent:(__ StatementList)? {
      return {
        type:       "SwitchCase",
        test:       null,
        consequent: optionalList(extractOptional(consequent, 1))
...
DebuggerStatement
  = T_DEBUGGER EOS { return { type: "DebuggerStatement" }; }

/* ----- A.5 Functions and Programs ----- */

FunctionExpression
  = T_FUNCTION __ id:(Identifier __)?
