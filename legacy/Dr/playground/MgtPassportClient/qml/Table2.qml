...
ColumnLayout{
    ...
    property alias fieldData: __dataModel.fieldData
            ...
            Component { // column delegate loader for instantitor
                ...
                Loader {
                    ...
                    source: if( hidden )                                        return ''
                            else if( modelData.tableDelegate === 'text' )            return 'qrc:/TableBasicColumn.qml'
                            else if( modelData.tableDelegate === 'int' )             return 'qrc:/TableIntColumn.qml'
                            else if( modelData.tableDelegate === 'real' )            return 'qrc:/TableRealColumn.qml'
                            else if( modelData.tableDelegate === 'date' )            return 'qrc:/TableDateColumn.qml'
                            else if( modelData.tableDelegate === 'bool' )            return 'qrc:/TableBoolColumn.qml'
                            else if( modelData.tableDelegate === 'transportType' )   return 'qrc:/TableTransportColumn.qml'
                            else                                                     return 'qrc:/TableBasicColumn.qml'
                    onLoaded: item.width = modelData.width
                }
            }
        }
...
//                        headerContextMenu.removeItem( object )
                    ...
                    MenuItem {
                        ...
                        onToggled: {
                            for( var i = 0; i < table.columnCount; ++i )
                                if( table.getColumn( i ).role === d.name ) {
        ...
        Menu { // table context menu
            ...
            MenuSeparator { visible: tableContextMenuCurrentItemText.text !== '' }
            MenuItem {
